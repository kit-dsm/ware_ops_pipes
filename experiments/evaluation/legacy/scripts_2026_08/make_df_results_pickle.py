...
def postprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Recompute layout_cache_hit: the sidecar persists after the first build,
    # so the per-summary value is unreliable.  A pipeline is a cache hit if
    # its layout_load_time matches another pipeline's (same layout).  Mark
    # the first occurrence per unique layout_load_time as the miss (False).
    if "layout_load_time" in df.columns and df["layout_load_time"].notna().any():
        df["layout_cache_hit"] = df.duplicated(subset=["layout_load_time"], keep="first")
        # Rows with NaN layout_load_time (legacy) stay NaN
        miss_mask = df["layout_load_time"].isna()
        df["layout_cache_hit"] = df["layout_cache_hit"].astype(object)
        df.loc[miss_mask, "layout_cache_hit"] = np.nan
