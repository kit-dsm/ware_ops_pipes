...
def _literature_instance_count(reference: pd.DataFrame, instance_col: str) -> int:
    """Count distinct instances for which the literature reports a reference value."""
    return int(
        reference.loc[
            reference["reference_value"].notna(),
            instance_col,
        ].nunique()
    )
...
def _validation_row(
    ...
    """
    Build one literature-comparison row.

    The evaluation population is defined by the literature: n_instances is the
    number of instances for which the cited source reports a reference value.
    n_feasible is the number of those instances for which the evaluated CASOP
    portfolio obtains at least one feasible solution. The mean gap is computed
    only over these feasible literature instances.
    """
    feasible = vbs_df[vbs_df["reference_value"].notna()].copy()
    n_feasible = int(feasible["instance_name"].nunique())

    if n_feasible > n_instances:
        raise ValueError(
            f"{set_label}: {n_feasible} feasible literature instances "
            f"but only {n_instances} literature instances."
        )
...
def generate_validation(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # total_distance defaults to 0 when routing did not run. Such rows are not
    # feasible distance solutions and must not win the VBS.
    ...
    ref_sprp = reference_from_solvers(
        ref_sprp,
        ["GS MIP cost", "Netw MIP cost", "DP cost"],
    )
    n_instances_sprp = _literature_instance_count(ref_sprp, "filename")
    ...
    ref_ss = reference_from_solvers(
        ref_ss,
        ["GS MIP cost", "Netw MIP cost"],
    )
    n_instances_ss = _literature_instance_count(ref_ss, "filename")
    ...
    n_instances_bo = _literature_instance_count(ref_bo, "filename")
    ...
    ref_hw["storage_policy"] = (
        ref_hw["filename"].str.split("\\").str[0].str.split("_").str[1]
    )
    ref_hw["filename"] = (
        ref_hw["filename"]
        .str.split("\\")
    ...
    ref_hw.loc[
        ref_hw["storage_policy"] == "uniform", "filename"
    ] = (
        ref_hw.loc[ref_hw["storage_policy"] == "uniform", "filename"] + "_u"
    )
    ref_hw.loc[
        ref_hw["storage_policy"] == "class-based", "filename"
    ] = (
        ref_hw.loc[ref_hw["storage_policy"] == "class-based", "filename"] + "_cb"
    )
    ...
    n_instances_hw = _literature_instance_count(ref_hw, "filename")
    ...
    # Keep only literature-reported instances for the VBS used in this
    # comparison. n_instances_hw was counted before looking at CASOP feasibility,
    # so missing/infeasible CASOP instances remain visible in the denominator.
    ...
    n_instances_mo = _literature_instance_count(ref_mo, "filename")
    ...
    n_instances_fm = _literature_instance_count(ref_fm, "Name")

    df_fm = valid[valid["instance_set"] == "FoodmartData"].copy()
    df_fm["instance_name"] = df_fm["instance_name"].str.replace(
        r"^instances_|_MAL$",
        "",
        regex=True,
    )
    ...
    vbs_fm = df_fm.loc[
        df_fm.groupby("instance_name")["total_distance"].idxmin()
    ].copy()
    rows.append(
        _validation_row(
            vbs_fm,
    ...
    # Kris (picking time under hard due-date feasibility)
    ...
    lines = [
        r"\begin{table}[tbp]",
        r"\centering",
        r"\caption{Comparison with solution values reported in the literature. "
        r"Feasible/Instances refers to the literature-reported instances; "
        r"gaps are means over feasible instances.}",
...
def _generate_kris_validation(df: pd.DataFrame) -> dict:
    ...
    if reference is None or reference.empty:
        return {
            ...
            "n_feasible": 0,
            "n_instances": 0,
            "gap_to_ref_[%]": float("nan"),
        }

    if "instance_name" not in reference.columns:
        raise KeyError(
            "Kris reference table must contain 'instance_name' so the "
            "literature evaluation population can be defined explicitly."
        )

    # Fix the population before looking at CASOP feasibility.
    ...
    # build_kris_table already identifies the due-date-feasible VBS instances
    # that have a published reference. Under the new table semantics, that
    # quantity is "Feasible", while n_instances is fixed by the literature.
    table = build_kris_table(vbs, n_instances)
    row = table.iloc[0]
    n_feasible = int(row["n_referenced"])

    if n_feasible > n_instances:
        raise ValueError(
            f"Kris: {n_feasible} feasible literature instances "
            f"but only {n_instances} literature instances."
        )
...
def _write_regret_exceedance_latex(summary: pd.DataFrame) -> None:
    ...
    lines = [
        r"\begin{table}[H]",
        r"\centering",
        r"\caption{SBS attainment and regret exceedance rates. Regret is in percent"
        r" for distance and time objectives and in percentage points for the on-time"
        r" rate. ``Pos.'' is the share of instances with positive regret;",
        r"$>x\%$ is the share exceeding threshold~$x$.}",
        ...
        r"\toprule",
        r"Set & Objective & SBS & Mean & Max & SBS at VBS & Pos. & "
