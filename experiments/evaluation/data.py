...
def normalize_instance_sets(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # The paper uses the Muter--Oencan instances rerun with the
    # Wahlen--Gschwind geometry.
    wg_muter = df["instance_set"].eq("MuterOencanWG")
    if wg_muter.any():
        df = df[~df["instance_set"].eq("MuterOencan")].copy()
        df.loc[df["instance_set"].eq("MuterOencanWG"), "instance_set"] = (
            "MuterOencan"
        )
