...
from kris_evaluation import run_kris_validation
...
KRIS_PUBLISHED_RESULTS_PATH = Path("../../data/results/briant_results/")
...
def create_row_vbs(
    ...
    subset, so # Referenced is the denominator throughout.
    """
...
# -100% gap. Drop these degenerate rows before any selection.
...
# =============================================================================
# Kris (JOBPRSP-D) — validated separately, see kris_validation.py
#
# Their objective is total time under hard deadlines, so it does not belong in
# the distance table. Gaps are measured only on deadline-feasible schedules,
# and the infeasible ones are quantified by a relaxation factor instead.
# =============================================================================

df_kris_vbs = run_kris_validation(
    df,
    small_dir=KRIS_SMALL_SOLUTIONS_PATH,
    large_dir=KRIS_LARGE_SOLUTIONS_PATH,
    published_dir=KRIS_PUBLISHED_RESULTS_PATH,
    n_evaluated=EVALUATED_COUNTS["Kris"],
)


# =============================================================================
# Validation table for the distance-based sets (VBS vs. reference)
#
# One row per instance set on the source's objective, which is travel distance
# for all six. Gap = (z_vbs - z_ref)/z_ref averaged over referenced instances,
# VBS = best synthesized pipeline per instance. z_ref is the value reported by
# the source paper for that set (see Table 1), NOT a surveyed best-known
# solution across all of the literature.
# =============================================================================
...
tab_validation = tab_validation[COLUMN_ORDER]
...
tab_validation.to_csv("tab_validation.csv", index=False)

# LaTeX gets display headers, italic set names, and -- for undefined opt gaps.
tab_tex = tab_validation.copy()
...
# context-aware synthesis. This is a separate object from the validation table
# (a distribution, not a single named pipeline).
# =============================================================================
...
# Kris winners are restricted to deadline-feasible schedules, so the
# distribution shows which pipelines actually produce admissible solutions.
if df_kris_vbs is not None:
    kris_feasible = df_kris_vbs[df_kris_vbs["feasible"] == True]  # noqa: E712
    if not kris_feasible.empty:
        winner_frames.append(kris_feasible[["instance_set", "strategy", "instance_name"]])
...
# Kris is total time under deadlines (different objective and constraint set),
# so it is not part of the distance gap-to-reference boxplot.
all_gaps = pd.concat([
...
plt.savefig(
    "./plots/vbs_gap_to_reference_boxplot_combined.png",
